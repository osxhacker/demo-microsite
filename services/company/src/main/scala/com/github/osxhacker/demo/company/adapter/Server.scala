...
import com.github.osxhacker.demo.company.adapter.kafka.AllConsumers
import com.github.osxhacker.demo.company.domain.GlobalEnvironment


/**
 * The '''Server''' `object` defines the entrypoint for the Company process.
 */
object Server
		...
		with ProgramArguments
{
	...
	override protected def main (mode : OperatingMode) : IO[ExitCode] =
		Monad[IO].flatMap2 (
		...
		: IO[ExitCode] =
	{
		...
		BlazeServerBuilder[IO].withIdleTimeout (settings.idleTimeout.value)
