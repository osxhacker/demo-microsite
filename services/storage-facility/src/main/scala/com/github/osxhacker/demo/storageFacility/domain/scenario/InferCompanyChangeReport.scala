...
/**
 * The '''InferCompanyChangeReport''' `object` defines the algorithm for
 * producing a report of significant changes relevant to the
 * [[com.github.osxhacker.demo.storageFacility.domain.Company]] entity.  Key to
 ...
 */
private[scenario] object InferCompanyChangeReport
	...
	private val slugChanged : (Company, Company) => Option[String] = {
		case (before, after) =>
			Option.unless (before.slug === after.slug) (
				new StringBuilder ()
					.append ("changed company slug from '")
	...
	private val statusChanged : (Company, Company) => Option[String] = {
		case (before, after) =>
			Option.unless (before.status === after.status) (
				new StringBuilder ()
					.append ("changed company status from '")
