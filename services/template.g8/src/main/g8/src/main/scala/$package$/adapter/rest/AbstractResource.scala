 ...
 * [[$package$.adapter.rest]] resources.
 */
	...
	extends ServiceResource[F] ()
{
			...
			env =>
				ServiceUnavailableDetails.from (
					title = env.shutdownMessage
						.getOrElse ("$name;format="norm"$ is offline")
					) |>
					mapIntoErrorType
			}
	 ...
	 * a [[$package$.adapter.rest.api.ProblemDetails]]
	 ...
	 * [[$package$.adapter.rest.api.LogicErrorDetails]]
	 * will be emitted.
	 */
